...
export default function StorePage() {
  ...
  return (
    <div className="flex flex-col h-screen bg-[#f8fafc] dark:bg-[#0b141a] overflow-hidden transition-colors duration-300">
      ...
      {/* HEADER DA LOJA */}
      <div className="bg-white dark:bg-[#0a0a0c] px-8 py-5 border-b border-slate-200 dark:border-[#27272a] flex justify-between items-center shrink-0 transition-colors">
         <div>
            <h1 className="text-2xl font-black text-slate-800 dark:text-[#fafafa] flex items-center gap-2">
               <Package className="w-6 h-6 text-rose-500" /> Loja & Estoque
            </h1>
            <p className="text-sm text-slate-400 dark:text-[#71717a] font-medium mt-1">Gestão de produtos, vendas e validade.</p>
         </div>

         {/* NAVEGAÇÃO ENTRE ABAS */}
         <div className="flex bg-slate-100 dark:bg-[#18181b] p-1 rounded-xl transition-colors">
            {[
               ...
               <button
                  ...
                  className={`flex items-center gap-2 px-6 py-2.5 rounded-lg text-sm font-bold transition-all ${
                     activeTab === tab.id 
                     ? 'bg-white dark:bg-[#0a0a0c] text-rose-600 dark:text-rose-400 shadow-sm' 
                     : 'text-slate-500 dark:text-[#a1a1aa] hover:text-slate-700 dark:hover:text-gray-200 hover:bg-slate-200/50 dark:hover:bg-white/5'
                  }`}
               >
      ...
      <div className="flex-1 overflow-hidden relative">
            ...
            <div className="h-full overflow-y-auto p-8 custom-scrollbar">
               ...
               {loadingMetrics ? (
                  <div className="grid grid-cols-4 gap-6 mb-8">
                     {[1, 2, 3, 4].map(i => (
                        <div key={i} className="bg-white dark:bg-[#0a0a0c] p-5 rounded-2xl border border-slate-100 dark:border-[#27272a] shadow-sm animate-pulse">
               ...
               <div className="bg-white dark:bg-[#0a0a0c] p-4 rounded-2xl border border-slate-100 dark:border-[#27272a] shadow-sm mb-6">
                  <div className="flex items-center gap-2 mb-3">
                    <ShieldCheck className="w-4 h-4 text-emerald-500" />
                    <h3 className="text-sm font-bold text-slate-700 dark:text-gray-200">Operações e Segurança</h3>
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
                    <button onClick={() => handleTabChange('pos')} className="px-3 py-2 rounded-lg border border-slate-200 dark:border-[#2e2e33] text-xs font-bold text-slate-600 dark:text-[#d4d4d8] hover:bg-slate-50 dark:hover:bg-white/5">Abrir checkout</button>
                    <button onClick={() => setShowScanner(true)} className="px-3 py-2 rounded-lg border border-slate-200 dark:border-[#2e2e33] text-xs font-bold text-slate-600 dark:text-[#d4d4d8] hover:bg-slate-50 dark:hover:bg-white/5">Scanner mobile</button>
                    <button onClick={() => handleTabChange('cash')} className="px-3 py-2 rounded-lg border border-slate-200 dark:border-[#2e2e33] text-xs font-bold text-slate-600 dark:text-[#d4d4d8] hover:bg-slate-50 dark:hover:bg-white/5">Fechar caixa loja</button>
                    <button onClick={exportOperationalReport} className="px-3 py-2 rounded-lg border border-slate-200 dark:border-[#2e2e33] text-xs font-bold text-slate-600 dark:text-[#d4d4d8] hover:bg-slate-50 dark:hover:bg-white/5">Exportar logs</button>
                    <button onClick={() => handleTabChange('catalog')} className="px-3 py-2 rounded-lg border border-slate-200 dark:border-[#2e2e33] text-xs font-bold text-slate-600 dark:text-[#d4d4d8] hover:bg-slate-50 dark:hover:bg-white/5">Auditoria catálogo</button>
                  </div>
               </div>

               <div className="grid grid-cols-3 gap-6">
                  {/* Gráfico de Vendas */}
                  <div className="col-span-2 bg-white dark:bg-[#0a0a0c] p-6 rounded-3xl border border-slate-100 dark:border-[#27272a] shadow-sm h-[400px] flex flex-col transition-colors">
                     <h3 className="font-bold text-slate-800 dark:text-[#fafafa] mb-6">Desempenho de Vendas (7 dias)</h3>
                  ...
                  <div className="bg-white dark:bg-[#0a0a0c] p-6 rounded-3xl border border-slate-100 dark:border-[#27272a] shadow-sm flex flex-col transition-colors">
                        ...
                        {loadingMetrics ? (
                           ...
                           metrics.expiringList.map((item: any, i: number) => (
                              <div key={i} className="flex justify-between items-center p-3 bg-rose-50 dark:bg-rose-900/10 rounded-xl border border-rose-100 dark:border-rose-900/20">
                                 ...
                                 <div className="bg-white dark:bg-[#0a0a0c] px-3 py-1 rounded-lg text-xs font-bold text-slate-600 dark:text-[#d4d4d8] shadow-sm">
                                    {item.quantity} un
                                 </div>
                              </div>
                           ))
                        ) : (
                           <div className="text-center text-slate-400 dark:text-[#71717a] text-sm py-8">
                     ...
                     <div className="mt-4 grid grid-cols-2 gap-2">
                        <button
                           ...
                           className="w-full py-2 text-xs font-bold text-slate-500 dark:text-[#a1a1aa] hover:text-rose-500 dark:hover:text-rose-400 transition-colors border border-slate-200 dark:border-[#2e2e33] rounded-lg"
                        >
                           Visão financeira
                        </button>
                        <button
         ...
         {activeTab === 'pos' && (
            <div className="h-full flex">
               ...
               <div className="flex-1 flex flex-col p-6 pr-3 border-r border-slate-200 dark:border-[#27272a]">
                  <div className="mb-6 flex gap-3">
                     <div className="relative flex-1">
                        <Search className="absolute left-3 top-3 w-5 h-5 text-slate-400 dark:text-[#71717a]" />
                        <input 
                           className="w-full pl-10 pr-4 py-3 bg-white dark:bg-[#0a0a0c] border border-slate-200 dark:border-[#2e2e33] rounded-xl focus:outline-none focus:ring-2 focus:ring-rose-500 dark:focus:ring-rose-900 transition-all shadow-sm text-slate-700 dark:text-gray-200 placeholder:text-slate-400 dark:placeholder:text-gray-600"
                     ...
                     <button
                        ...
                        className="px-4 py-3 bg-white dark:bg-[#0a0a0c] border border-slate-200 dark:border-[#2e2e33] rounded-xl font-bold text-sm text-slate-700 dark:text-[#d4d4d8] hover:bg-slate-50 dark:hover:bg-white/5 transition-all flex items-center gap-2"
                     >
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar">
                     <div className="grid grid-cols-3 xl:grid-cols-4 gap-4">
                        {products.filter(p => p.name.toLowerCase().includes(searchTerm.toLowerCase())).map(product => (
                           <div key={product.id} onClick={() => addToCart(product)} className="bg-white dark:bg-[#0a0a0c] p-4 rounded-2xl border border-slate-100 dark:border-[#2e2e33] shadow-sm hover:border-rose-300 dark:hover:border-rose-700 hover:shadow-md cursor-pointer transition-all group relative overflow-hidden flex flex-col h-[200px]">
                              <div className="flex-1 bg-slate-50 dark:bg-[#18181b] rounded-xl mb-3 flex items-center justify-center relative overflow-hidden">
                                 {product.image_url ? (
                                    <img src={product.image_url} className="w-full h-full object-cover" />
                                 ) : (
                                    <div className="flex flex-col items-center">
                                       <Package className="w-8 h-8 text-slate-300 dark:text-gray-600 mb-1" />
                                       <span className="text-[10px] text-slate-400 dark:text-[#71717a] font-mono">{product.barcode || 'S/ BARCODE'}</span>
                                    </div>
                                 )}
                                 <div className="absolute inset-0 bg-rose-500/10 dark:bg-rose-500/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                    <Plus className="w-8 h-8 text-rose-600 dark:text-rose-400 bg-white dark:bg-[#0a0a0c] rounded-full p-1 shadow-sm" />
                                 </div>
                              </div>
                              <div>
               ...
               {/* Direita: Carrinho */}
               <div className="w-[420px] bg-white dark:bg-[#0a0a0c] flex flex-col z-10 shadow-xl border-l border-slate-100 dark:border-[#27272a] transition-colors">
                  {/* Cliente */}
                  <div className="p-6 border-b border-slate-100 dark:border-[#27272a]">
                     <label className="block text-xs font-bold text-slate-400 dark:text-[#71717a] uppercase mb-2">Vincular Paciente (Opcional)</label>
                     <div className="relative">
                        <Search className="absolute left-3 top-3.5 w-4 h-4 text-slate-400 dark:text-[#71717a]" />
                        <input
                           type="text"
                           className="w-full pl-10 pr-4 py-3 bg-slate-50 dark:bg-[#18181b] border border-slate-200 dark:border-[#2e2e33] rounded-xl text-sm font-bold text-slate-700 dark:text-gray-200 focus:outline-none focus:border-rose-400 dark:focus:border-rose-700"
                     ...
                     {patientSearchTerm && (
                        <div className="mt-2 max-h-48 overflow-y-auto custom-scrollbar border border-slate-200 dark:border-[#2e2e33] rounded-lg bg-white dark:bg-[#0a0a0c]">
                           {patients
                              ...
                              .map(p => (
                                 <button
                                    ...
                                    className={`w-full text-left px-4 py-2.5 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors border-b border-slate-100 dark:border-[#27272a] last:border-0 ${
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                     {cart.length === 0 ? (
                        ...
                        <div key={item.id} className="flex gap-3 bg-white dark:bg-[#18181b] p-3 rounded-xl border border-slate-100 dark:border-[#2e2e33] shadow-sm items-center">
                           ...
                           <div className="flex items-center gap-1">
                              <button onClick={() => updateQty(item.id, -1)} className="p-1 hover:bg-slate-100 dark:hover:bg-white/10 rounded text-slate-400 dark:text-[#71717a] hover:text-slate-600 dark:hover:text-gray-300">-</button>
                              <button onClick={() => updateQty(item.id, 1)} className="p-1 hover:bg-slate-100 dark:hover:bg-white/10 rounded text-slate-400 dark:text-[#71717a] hover:text-slate-600 dark:hover:text-gray-300">+</button>
                  ...
                  {/* Totais e Ações */}
                  <div className="p-6 bg-slate-50 dark:bg-[#111b21] border-t border-slate-200 dark:border-[#27272a] transition-colors">
                     <div className="flex justify-between items-center mb-6">
                        <span className="text-slate-500 dark:text-[#a1a1aa] font-bold">Total a Pagar</span>
                        <span className="text-3xl font-black text-rose-600 dark:text-rose-400">R$ {cartTotal.toFixed(2)}</span>
                     </div>
                     
                    {/* Pagamento inline */}
                    <div className="mb-3">
                      <p className="text-xs font-bold text-slate-500 dark:text-[#a1a1aa] uppercase mb-2">Forma de pagamento</p>
                      <div className="grid grid-cols-2 gap-2 mb-2">
                        <button onClick={() => setSelectedPaymentMethod('cash')} className={`py-2 rounded-lg font-bold text-xs border ${selectedPaymentMethod === 'cash' ? 'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-300 dark:border-emerald-800 text-emerald-700 dark:text-emerald-400' : 'bg-white dark:bg-[#18181b] border-slate-200 dark:border-[#2e2e33] text-slate-600 dark:text-[#d4d4d8]'}`}>Dinheiro</button>
                        <button onClick={() => setSelectedPaymentMethod('pix')} className={`py-2 rounded-lg font-bold text-xs border ${selectedPaymentMethod === 'pix' ? 'bg-teal-50 dark:bg-teal-900/20 border-teal-300 dark:border-teal-800 text-teal-700 dark:text-teal-400' : 'bg-white dark:bg-[#18181b] border-slate-200 dark:border-[#2e2e33] text-slate-600 dark:text-[#d4d4d8]'}`}>Pix</button>
                      </div>
                      <div className="grid grid-cols-3 gap-2">
                        <button onClick={() => setSelectedPaymentMethod('credit_card')} className={`py-2 rounded-lg font-bold text-xs border ${selectedPaymentMethod === 'credit_card' ? 'bg-slate-100 dark:bg-slate-700 border-slate-400 dark:border-slate-500 text-slate-800 dark:text-white' : 'bg-white dark:bg-[#18181b] border-slate-200 dark:border-[#2e2e33] text-slate-600 dark:text-[#d4d4d8]'}`}>Crédito</button>
                        <button onClick={() => setSelectedPaymentMethod('debit_card')} className={`py-2 rounded-lg font-bold text-xs border ${selectedPaymentMethod === 'debit_card' ? 'bg-indigo-50 dark:bg-indigo-900/20 border-indigo-300 dark:border-indigo-800 text-indigo-700 dark:text-indigo-300' : 'bg-white dark:bg-[#18181b] border-slate-200 dark:border-[#2e2e33] text-slate-600 dark:text-[#d4d4d8]'}`}>Débito</button>
                        <button onClick={() => setSelectedPaymentMethod('mixed')} className={`py-2 rounded-lg font-bold text-xs border ${selectedPaymentMethod === 'mixed' ? 'bg-amber-50 dark:bg-amber-900/20 border-amber-300 dark:border-amber-800 text-amber-700 dark:text-amber-400' : 'bg-white dark:bg-[#18181b] border-slate-200 dark:border-[#2e2e33] text-slate-600 dark:text-[#d4d4d8]'}`}>Misto</button>
                      </div>
                    </div>

                    {selectedPaymentMethod === 'cash' && (
                        <div className="mb-4 p-4 bg-white dark:bg-[#18181b] rounded-xl border border-slate-200 dark:border-[#2e2e33]">
                           <label className="block text-xs font-bold text-slate-500 dark:text-[#a1a1aa] mb-2">Valor Recebido</label>
                           <input
                              type="number"
                              step="0.01"
                              min={cartTotal}
                              ...
                              className="w-full px-4 py-2.5 bg-slate-50 dark:bg-[#0a0a0c] border border-slate-200 dark:border-[#2e2e33] rounded-lg text-lg font-bold text-slate-700 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-rose-500"
                              placeholder="0.00"
                           />
                           {parseFloat(cashAmount || '0') >= cartTotal && (
                              <div className="mt-3 pt-3 border-t border-slate-200 dark:border-[#2e2e33]">
                                 <div className="flex justify-between items-center">
                                    <span className="text-sm font-bold text-slate-600 dark:text-[#a1a1aa]">Troco</span>
                    ...
                    {selectedPaymentMethod === 'mixed' && (
                      <div className="mb-3 p-3 bg-white dark:bg-[#18181b] rounded-xl border border-slate-200 dark:border-[#2e2e33]">
                        <div className="grid grid-cols-2 gap-2">
                          <input type="number" step="0.01" min="0" value={mixedAmounts.pix} onChange={(e) => setMixedAmounts((prev) => ({ ...prev, pix: e.target.value }))} className="px-2 py-2 bg-slate-50 dark:bg-[#0a0a0c] border border-slate-200 dark:border-[#2e2e33] rounded text-xs font-bold" placeholder="Pix" />
                          <input type="number" step="0.01" min="0" value={mixedAmounts.cash} onChange={(e) => setMixedAmounts((prev) => ({ ...prev, cash: e.target.value }))} className="px-2 py-2 bg-slate-50 dark:bg-[#0a0a0c] border border-slate-200 dark:border-[#2e2e33] rounded text-xs font-bold" placeholder="Dinheiro" />
                          <input type="number" step="0.01" min="0" value={mixedAmounts.credit_card} onChange={(e) => setMixedAmounts((prev) => ({ ...prev, credit_card: e.target.value }))} className="px-2 py-2 bg-slate-50 dark:bg-[#0a0a0c] border border-slate-200 dark:border-[#2e2e33] rounded text-xs font-bold" placeholder="Crédito" />
                          <input type="number" step="0.01" min="0" value={mixedAmounts.debit_card} onChange={(e) => setMixedAmounts((prev) => ({ ...prev, debit_card: e.target.value }))} className="px-2 py-2 bg-slate-50 dark:bg-[#0a0a0c] border border-slate-200 dark:border-[#2e2e33] rounded text-xs font-bold" placeholder="Débito" />
                        </div>
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        ...
                        className="py-3 bg-white dark:bg-[#18181b] border border-slate-200 dark:border-[#2e2e33] rounded-xl font-bold text-slate-600 dark:text-[#d4d4d8] text-sm hover:bg-slate-50 dark:hover:bg-white/5 transition-all"
                      >
                        Consumidor final
                      </button>
                      <button
         ...
         {activeTab === 'catalog' && (
            <div className="h-full p-8 overflow-y-auto custom-scrollbar">
               <div className="flex justify-between items-center mb-6">
                  <h2 className="text-xl font-bold text-slate-800 dark:text-[#fafafa]">Gerenciar Produtos</h2>
                  <div className="flex gap-3">
                     <button 
                        onClick={fetchData}
                        className="bg-slate-100 dark:bg-[#18181b] text-slate-700 dark:text-[#d4d4d8] px-4 py-2.5 rounded-xl font-bold text-sm hover:bg-slate-200 dark:hover:bg-white/10 flex items-center gap-2 transition-all"
               ...
               <div className="bg-white dark:bg-[#0a0a0c] rounded-2xl border border-slate-200 dark:border-[#27272a] p-4 mb-4 grid grid-cols-4 gap-3">
                  <input
                     value={catalogSearchTerm}
                     onChange={(e) => setCatalogSearchTerm(e.target.value)}
                     placeholder="Buscar por nome ou código"
                     className="col-span-2 px-3 py-2 rounded-lg bg-slate-50 dark:bg-[#18181b] border border-slate-200 dark:border-[#2e2e33] text-sm font-semibold text-slate-700 dark:text-gray-200"
                  />
                  <select
                    value={catalogCategoryFilter}
                    onChange={(e) => setCatalogCategoryFilter(e.target.value)}
                    className="px-3 py-2 rounded-lg bg-slate-50 dark:bg-[#18181b] border border-slate-200 dark:border-[#2e2e33] text-sm font-semibold text-slate-700 dark:text-gray-200"
                  >
               ...
               <div className="bg-white dark:bg-[#0a0a0c] rounded-3xl border border-slate-200 dark:border-[#27272a] shadow-sm overflow-hidden transition-colors">
                  <table className="w-full text-left">
                     <thead className="bg-slate-50 dark:bg-[#18181b] border-b border-slate-100 dark:border-[#2e2e33]">
                        <tr>
                           <th className="p-4 text-xs font-bold text-slate-500 dark:text-[#a1a1aa] uppercase">Produto</th>
                           <th className="p-4 text-xs font-bold text-slate-500 dark:text-[#a1a1aa] uppercase">Categoria</th>
                           <th className="p-4 text-xs font-bold text-slate-500 dark:text-[#a1a1aa] uppercase text-right">Custo</th>
                           <th className="p-4 text-xs font-bold text-slate-500 dark:text-[#a1a1aa] uppercase text-right">Venda</th>
                           <th className="p-4 text-xs font-bold text-slate-500 dark:text-[#a1a1aa] uppercase text-center">Estoque</th>
                           <th className="p-4 text-xs font-bold text-slate-500 dark:text-[#a1a1aa] uppercase text-center">Ações</th>
                        </tr>
                     </thead>
                              ...
                              <td className="p-4">
                                 <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                       ...
                                       <p className="text-[10px] text-slate-400 dark:text-[#71717a] font-mono">{product.barcode || 'Sem EAN'}</p>
                                    </div>
                                 </div>
                              </td>
                              <td className="p-4">
                                 <span className="bg-slate-100 dark:bg-[#111b21] text-slate-600 dark:text-[#a1a1aa] px-2 py-1 rounded text-xs font-bold capitalize">{product.category || 'Geral'}</span>
                              </td>
                              <td className="p-4 text-right text-sm text-slate-500 dark:text-[#a1a1aa] font-medium">R$ {product.price_cost?.toFixed(2)}</td>
         ...
         {activeTab === 'cash' && (
            <div className="h-full p-8 overflow-y-auto custom-scrollbar">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h2 className="text-xl font-bold text-slate-800 dark:text-[#fafafa]">Caixa da Lojinha</h2>
                  <p className="text-sm text-slate-500 dark:text-[#a1a1aa]">Fechamento dedicado da operação da loja com reconciliação por método.</p>
                </div>
                <div className="flex gap-2">
                  <button onClick={fetchStoreCash} className="px-4 py-2 rounded-xl border border-slate-200 dark:border-[#2e2e33] text-sm font-bold text-slate-600 dark:text-[#d4d4d8] hover:bg-slate-50 dark:hover:bg-white/5">
                    Atualizar
                  </button>
                  <button
              ...
              {loadingStoreCash ? (
                <div className="bg-white dark:bg-[#0a0a0c] rounded-2xl border border-slate-100 dark:border-[#27272a] p-8 text-center text-slate-500 dark:text-[#a1a1aa]">
                ...
                <>
                  ...
                  <div className="bg-white dark:bg-[#0a0a0c] rounded-2xl border border-slate-100 dark:border-[#27272a] shadow-sm overflow-hidden">
                    <div className="px-5 py-4 border-b border-slate-100 dark:border-[#27272a] flex items-center justify-between">
                      <h3 className="font-bold text-slate-700 dark:text-gray-200">Logs da sessão da loja</h3>
                      <span className="text-xs font-bold text-slate-500 dark:text-[#a1a1aa]">{storeCashData.transactions.length} lançamentos</span>
                    </div>
                    <div className="max-h-[420px] overflow-y-auto custom-scrollbar">
                      {storeCashData.transactions.length === 0 ? (
                        <div className="p-8 text-center text-slate-500 dark:text-[#a1a1aa]">Sem transações da loja para a data selecionada.</div>
                      ) : (
                        <table className="w-full text-sm">
                          ...
                          <tbody>
                            {storeCashData.transactions.map((transaction) => (
                              <tr key={transaction.id} className="border-t border-slate-100 dark:border-[#27272a]">
                                <td className="px-4 py-3 text-slate-700 dark:text-[#d4d4d8]">{new Date(transaction.occurred_at).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</td>
                                <td className="px-4 py-3 font-bold text-slate-800 dark:text-[#fafafa]">R$ {Number(transaction.amount || 0).toFixed(2)}</td>
                                <td className="px-4 py-3 text-slate-600 dark:text-[#a1a1aa]">{transaction.notes || '-'}</td>
        ...
        {showScanner && (
           ...
           >
              <div
                 className="bg-white dark:bg-[#0a0a0c] rounded-3xl shadow-2xl max-w-md w-full p-6"
                 onClick={(e) => e.stopPropagation()}
              >
                 <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-black text-slate-800 dark:text-[#fafafa]">Scanner no celular</h3>
                    <button
                       type="button"
                       onClick={() => setShowScanner(false)}
                       className="p-2 hover:bg-slate-100 dark:hover:bg-white/10 rounded-lg text-slate-400 dark:text-[#71717a]"
                    >
                       <X className="w-4 h-4" />
                    </button>
                 </div>
                 <p className="text-sm text-slate-500 dark:text-[#a1a1aa] mb-4">
                    Escaneie para abrir o scanner no celular e enviar produtos em tempo real para o carrinho.
                 </p>
                 <div className="bg-slate-50 dark:bg-[#18181b] border border-slate-200 dark:border-[#2e2e33] rounded-2xl p-4 flex items-center justify-center mb-3">
...
function MetricCard({ title, value, icon: Icon, color, trend, subtext }: any) {
   ...
   return (
      <div className="bg-white dark:bg-[#0a0a0c] p-5 rounded-2xl border border-slate-100 dark:border-[#27272a] shadow-sm flex flex-col transition-colors">
         ...
         <p className="text-slate-400 dark:text-[#71717a] text-xs font-bold uppercase tracking-wider">{title}</p>
         <p className="text-2xl font-black text-slate-800 dark:text-[#fafafa] mt-1">{value}</p>
         {subtext && <p className="text-xs text-slate-400 dark:text-[#71717a] mt-1 font-medium">{subtext}</p>}
      </div>
   )
}