...
export default function AgendaSidebar({ currentDate, setCurrentDate }: AgendaSidebarProps) {
  ...
  return (
    <div className="w-72 flex flex-col gap-4">
      <div className="bg-white dark:bg-[#141419] p-5 rounded-2xl border border-slate-100 dark:border-[#2d2d36] shadow-sm transition-colors">
          ...
          {daysInMonth.map((d, i) => {
            ...
            return (
              <button
                ...
                className={`h-8 w-8 rounded-lg text-xs font-bold transition-all flex items-center justify-center ${isSelected ? 'bg-rose-500 text-white shadow-md' : 'text-slate-600 dark:text-[#a1a1aa] hover:bg-slate-50 dark:hover:bg-white/10'}`}
              >
                {d.getDate()}
              </button>
            );
