              ...
              <Link
                ...
                className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-colors ${
                  active
                    ...
                    : 'text-slate-600 dark:text-[#828ca5] hover:bg-slate-50 dark:hover:bg-white/5 hover:text-slate-800 dark:hover:text-gray-200'
                }`}
              >
                <span className={active ? 'text-teal-500' : 'text-slate-400 dark:text-[#565d73]'}>
                  {item.icon}
                </span>
                {item.label}
              </Link>
...
export default function CadastrosLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-full overflow-hidden">
      {/* Sidebar */}
      <aside
        className={`shrink-0 flex flex-col bg-white dark:bg-[#0d0f15] border-r border-slate-200 dark:border-[#252a3a] transition-all duration-300 overflow-hidden ${
          collapsed ? 'w-0 min-w-0' : 'w-60 min-w-60'
        }`}
      >
        {/* Sidebar header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-[#1e2334]">
        ...
        <nav className="flex-1 overflow-y-auto custom-scrollbar px-2 py-3 space-y-1">
          ...
          <div className="pt-1">
            ...
            >
              <span className={pathname.startsWith(STANDALONE_ITEM.href) ? 'text-teal-500' : 'text-slate-400 dark:text-[#565d73]'}>
      ...
      {collapsed && (
        <button
          ...
          className="shrink-0 flex items-center justify-center w-8 border-r border-slate-200 dark:border-[#252a3a] bg-white dark:bg-[#0d0f15] text-slate-400 hover:text-teal-600 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors"
          title="Expandir menu"
        >
