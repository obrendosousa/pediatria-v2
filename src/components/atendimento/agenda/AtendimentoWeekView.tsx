    ...
    <div className="h-full flex overflow-x-auto custom-scrollbar">
      <div className="flex-1 grid grid-cols-7 min-w-[1600px] divide-x divide-slate-100 dark:divide-gray-800">
        {weekDays.map((day) => {
          ...
          return (
            <div key={dateStr} className={`flex flex-col h-full transition-colors ${isToday ? 'bg-teal-50/10 dark:bg-teal-900/5' : 'bg-white dark:bg-[#1e2028]'}`}>
              <div className={`text-center p-3 border-b ${isToday ? 'border-teal-200 dark:border-teal-900/30 bg-teal-50/50 dark:bg-teal-900/10' : 'border-slate-100 dark:border-gray-800'}`}>
                <span className={`text-[10px] font-extrabold uppercase block mb-1 tracking-wider ${isToday ? 'text-teal-600 dark:text-teal-400' : 'text-slate-400 dark:text-gray-500'}`}>{day.toLocaleDateString('pt-BR', { weekday: 'short' })}</span>
