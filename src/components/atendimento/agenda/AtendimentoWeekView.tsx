    ...
    <div className="h-full flex overflow-x-auto custom-scrollbar">
      <div className="flex-1 grid grid-cols-7 min-w-[1600px] divide-x divide-slate-100 dark:divide-gray-800">
        {weekDays.map((day) => {
          ...
          return (
            <div key={dateStr} className={`flex flex-col h-full transition-colors ${isToday ? 'bg-teal-50/10 dark:bg-teal-900/5' : 'bg-white dark:bg-[#0a0a0c]'}`}>
              <div className={`text-center p-3 border-b ${isToday ? 'border-teal-200 dark:border-teal-900/30 bg-teal-50/50 dark:bg-teal-900/10' : 'border-slate-100 dark:border-[#27272a]'}`}>
                <span className={`text-[10px] font-extrabold uppercase block mb-1 tracking-wider ${isToday ? 'text-teal-600 dark:text-teal-400' : 'text-slate-400 dark:text-[#71717a]'}`}>{day.toLocaleDateString('pt-BR', { weekday: 'short' })}</span>
                <div className={`inline-flex items-center justify-center w-8 h-8 rounded-full text-lg font-bold ${isToday ? 'bg-teal-600 text-white shadow-md' : 'text-slate-700 dark:text-[#a1a1aa]'}`}>{day.getDate()}</div>
              </div>
              <div className="flex-1 p-2 space-y-2 overflow-y-auto custom-scrollbar">
                {/* Bloqueios do dia */}
                {(dayBlocks?.[dateStr] || []).map((block, idx) => (
                  <div key={`block-${idx}`} className="p-2 rounded-lg bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-[#2e2e33] flex items-center gap-1.5 text-slate-400 dark:text-[#71717a]">
                ...
                {dayApps.map(app => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {total > 0 && (
                        <div className="flex flex-col gap-1 mt-1 pt-1 border-t border-slate-200/50 dark:border-[#2e2e33]/50">
                          {remaining <= 0 ? (
                ...
                <button onClick={() => openNewSlotModal(dateStr)} className="w-full py-2 border border-dashed border-slate-200 dark:border-[#2e2e33] rounded-lg text-slate-300 dark:text-gray-600 hover:text-teal-500 dark:hover:text-teal-400 hover:border-teal-300 dark:hover:border-teal-500/50 transition-all flex items-center justify-center"><Plus className="w-4 h-4" /></button>
              </div>
            </div>
          );
        })}
