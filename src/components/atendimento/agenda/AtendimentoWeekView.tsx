    ...
    <div className="h-full flex overflow-x-auto custom-scrollbar">
      <div className="flex-1 grid grid-cols-7 min-w-[1600px] divide-x divide-slate-100 dark:divide-gray-800">
        {weekDays.map((day) => {
          ...
          return (
            <div key={dateStr} className={`flex flex-col h-full transition-colors ${isToday ? 'bg-teal-50/10 dark:bg-teal-900/5' : 'bg-white dark:bg-[#0d0f15]'}`}>
              <div className={`text-center p-3 border-b ${isToday ? 'border-teal-200 dark:border-teal-900/30 bg-teal-50/50 dark:bg-teal-900/10' : 'border-slate-100 dark:border-[#1e2334]'}`}>
                <span className={`text-[10px] font-extrabold uppercase block mb-1 tracking-wider ${isToday ? 'text-teal-600 dark:text-teal-400' : 'text-slate-400 dark:text-[#565d73]'}`}>{day.toLocaleDateString('pt-BR', { weekday: 'short' })}</span>
                <div className={`inline-flex items-center justify-center w-8 h-8 rounded-full text-lg font-bold ${isToday ? 'bg-teal-600 text-white shadow-md' : 'text-slate-700 dark:text-[#828ca5]'}`}>{day.getDate()}</div>
              </div>
              <div className="flex-1 p-2 space-y-2 overflow-y-auto custom-scrollbar">
                {/* Bloqueios do dia */}
                {(dayBlocks?.[dateStr] || []).map((block, idx) => (
                  <div key={`block-${idx}`} className="p-2 rounded-lg bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-[#252a3a] flex items-center gap-1.5 text-slate-400 dark:text-[#565d73]">
