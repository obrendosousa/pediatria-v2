      ...
      <div className="bg-white dark:bg-[#0d0f15] rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="p-4 border-b border-gray-200 dark:border-[#252a3a] bg-gray-50 dark:bg-[#141722] flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div className="flex gap-2 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg">
              <button type="button" onClick={() => setSlotType('booked')} className={`px-3 py-1.5 text-xs font-semibold rounded-md transition-all flex items-center gap-1.5 ${slotType === 'booked' ? 'bg-white text-teal-600 shadow-sm dark:bg-[#141722] dark:text-teal-400' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400'}`}>
                <User size={14}/> Agendar
              </button>
              <button type="button" onClick={() => setSlotType('blocked')} className={`px-3 py-1.5 text-xs font-semibold rounded-md transition-all flex items-center gap-1.5 ${slotType === 'blocked' ? 'bg-white text-red-600 shadow-sm dark:bg-[#141722] dark:text-red-400' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400'}`}>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-5 custom-scrollbar">
          {slotType === 'booked' ? (
            <>
              <div>
                <label className="block text-xs font-bold text-gray-500 dark:text-[#828ca5] uppercase mb-1">Nome do Paciente *</label>
                <div className="relative">
                  <User className="w-4 h-4 text-gray-400 absolute left-3 top-3.5" />
                  <input type="text" value={formData.patient_name} onChange={e => setFormData({...formData, patient_name: e.target.value})} className="w-full pl-9 pr-3 py-2.5 border border-gray-200 dark:border-[#252a3a] rounded-lg bg-white dark:bg-[#141722] text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all" placeholder="Nome do paciente" required />
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-gray-500 dark:text-[#828ca5] uppercase mb-1">Responsavel</label>
                  <div className="relative">
                    <User className="w-4 h-4 text-gray-400 absolute left-3 top-3.5" />
                    <input type="text" value={formData.parent_name} onChange={e => setFormData({...formData, parent_name: e.target.value})} className="w-full pl-9 pr-3 py-2.5 border border-gray-200 dark:border-[#252a3a] rounded-lg bg-white dark:bg-[#141722] text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all" placeholder="Nome do responsavel" />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-500 dark:text-[#828ca5] uppercase mb-1">Sexo</label>
                  <div className="flex gap-0 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg">
                    <button type="button" onClick={() => setFormData({...formData, patient_sex: 'M'})} className={`flex-1 py-1.5 text-xs font-medium rounded-md transition-all shadow-sm ${formData.patient_sex === 'M' ? 'bg-white text-blue-600 shadow-sm dark:bg-[#141722] dark:text-blue-400' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400'}`}>Masculino</button>
                    <button type="button" onClick={() => setFormData({...formData, patient_sex: 'F'})} className={`flex-1 py-1.5 text-xs font-medium rounded-md transition-all shadow-sm ${formData.patient_sex === 'F' ? 'bg-white text-pink-600 shadow-sm dark:bg-[#141722] dark:text-pink-400' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400'}`}>Feminino</button>
                  </div>
                </div>
              </div>

              <div>
                <label className="block text-xs font-bold text-gray-500 dark:text-[#828ca5] uppercase mb-1">Data de Nascimento (DD/MM/AAAA)</label>
                <div className="relative">
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 dark:text-[#828ca5] uppercase mb-1">Telefone / WhatsApp</label>
                <div className="relative">
                  <Phone className="w-4 h-4 text-gray-400 absolute left-3 top-3.5" />
                  <input type="text" value={formData.parent_phone} onChange={e => setFormData({...formData, parent_phone: e.target.value})} className="w-full pl-9 pr-3 py-2.5 border border-gray-200 dark:border-[#252a3a] rounded-lg bg-white dark:bg-[#141722] text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all" placeholder="11999999999" />
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-gray-500 dark:text-[#828ca5] uppercase mb-1">Data * (DD/MM/AAAA)</label>
                  <div className="relative">
                    <Calendar className="w-4 h-4 text-gray-400 absolute left-3 top-3.5" />
                    <input type="text" value={formData.dateDisplay} onChange={e => handleDateMaskedInput(e.target.value, 'dateDisplay', 'date')} placeholder="DD/MM/AAAA" maxLength={10} className="w-full pl-9 pr-3 py-2.5 border border-gray-200 dark:border-[#252a3a] rounded-lg bg-white dark:bg-[#141722] text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all" required />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-500 dark:text-[#828ca5] uppercase mb-1">Horario *</label>
                  <div className="relative">
                    <Clock className="w-4 h-4 text-gray-400 absolute left-3 top-3.5" />
                    <input type="time" value={formData.time} onChange={e => setFormData({...formData, time: e.target.value})} className="w-full pl-9 pr-3 py-2.5 border border-gray-200 dark:border-[#252a3a] rounded-lg bg-white dark:bg-[#141722] text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all" required />
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <select value={selectedDoctorId || ''} onChange={e => setSelectedDoctorId(Number(e.target.value))} className="w-full pl-9 pr-3 py-2.5 border border-gray-200 dark:border-[#252a3a] rounded-lg bg-white dark:bg-[#141722] text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all" required>
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 dark:text-[#828ca5] uppercase mb-1">Tipo de Atendimento *</label>
                <select value={formData.type} onChange={e => setFormData({ ...formData, type: e.target.value as 'consulta' | 'retorno' | '' })} className="w-full px-3 py-2.5 border border-gray-200 dark:border-[#252a3a] rounded-lg bg-white dark:bg-[#141722] text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all" required>
              ...
              <div className="bg-slate-50 dark:bg-[#141722]/50 p-4 rounded-xl border border-slate-200 dark:border-[#252a3a] space-y-4">
                <h4 className="text-sm font-bold text-slate-700 dark:text-[#a0a8be] flex items-center gap-2"><Wallet className="w-4 h-4 text-emerald-500" /> Financeiro</h4>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-gray-500 dark:text-[#828ca5] uppercase mb-1">Valor Total (R$)</label>
                    <div className="relative">
                      <span className="absolute left-3 top-2.5 text-gray-500 dark:text-[#828ca5] font-bold text-sm">R$</span>
                      <input type="text" value={formData.totalAmount} onChange={e => handleMoneyInput('totalAmount', e.target.value)} className="w-full pl-9 pr-3 py-2 border border-gray-200 dark:border-[#252a3a] rounded-lg bg-white dark:bg-[#141722] text-gray-700 dark:text-gray-200 font-bold focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all" placeholder="0,00" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-500 dark:text-[#828ca5] uppercase mb-1">Entrada / Pago (R$)</label>
                    <div className="relative">
                ...
                {totalNum > 0 && (
                  <div className="flex justify-between items-center pt-2 border-t border-slate-200 dark:border-[#252a3a]">
                    <span className="text-sm text-slate-500 dark:text-[#828ca5]">Restante:</span>
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 dark:text-[#828ca5] uppercase mb-1">Observacoes</label>
                <div className="relative">
                  <FileText className="w-4 h-4 text-gray-400 absolute left-3 top-3.5" />
                  <textarea value={formData.notes} onChange={e => setFormData({...formData, notes: e.target.value})} className="w-full pl-9 pr-3 py-2.5 border border-gray-200 dark:border-[#252a3a] rounded-lg bg-white dark:bg-[#141722] text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all resize-none" placeholder="Observacoes..." rows={3} />
                </div>
              </div>
            </>
          ) : (
            <>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative">
                    <Calendar className="w-4 h-4 text-gray-400 absolute left-3 top-3.5" />
                    <input type="text" value={formData.dateDisplay} onChange={e => handleDateMaskedInput(e.target.value, 'dateDisplay', 'date')} placeholder="DD/MM/AAAA" maxLength={10} className="w-full pl-9 pr-3 py-2.5 border border-gray-200 dark:border-[#252a3a] rounded-lg bg-white dark:bg-[#141722] text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all" required />
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-500 dark:text-[#828ca5] uppercase mb-1">Horario *</label>
                  <div className="relative">
                    <Clock className="w-4 h-4 text-gray-400 absolute left-3 top-3.5" />
                    <input type="time" value={formData.time} onChange={e => setFormData({...formData, time: e.target.value})} className="w-full pl-9 pr-3 py-2.5 border border-gray-200 dark:border-[#252a3a] rounded-lg bg-white dark:bg-[#141722] text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all" required />
                  </div>
                </div>
              </div>
              <div>
        ...
        <div className="p-4 border-t border-gray-200 dark:border-[#252a3a] bg-gray-50 dark:bg-[#141722] flex justify-end gap-3">
          <button onClick={onClose} className="px-4 py-2 text-gray-500 dark:text-[#828ca5] hover:bg-gray-100 dark:hover:bg-white/10 rounded-lg text-sm font-bold transition-colors">Cancelar</button>
