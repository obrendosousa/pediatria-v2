  ...
  return (
    ...
    <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-white dark:bg-[#202c33] w-full max-w-lg rounded-2xl shadow-lg border border-slate-200 dark:border-[#252a3a] overflow-hidden animate-scale-in">
        <div className={`p-4 border-b border-slate-200 dark:border-[#252a3a] flex justify-between items-center ${
        ...
        <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto custom-scrollbar">
            ...
            <div className="flex-1">
              {isEditing ? (
                <div className="space-y-1">
                  ...
                  <input type="text" className="w-full text-sm font-medium text-slate-800 dark:text-[#e8ecf4] border border-slate-200 dark:border-gray-600 rounded-lg px-3 py-1.5 focus:border-teal-400 focus:ring-1 focus:ring-teal-400/20 outline-none bg-white dark:bg-[#141722] transition-all" value={editForm.patient_name} onChange={(e) => setEditForm({ ...editForm, patient_name: e.target.value })} />
                </div>
              ) : (
                <>
                  <h4 className="text-base font-semibold text-slate-800 dark:text-[#e8ecf4] mb-1.5">
              ...
              {isEditing && (
                <div className="space-y-2 mt-2">
                  <div>
                    <label className="text-[10px] font-semibold text-slate-500 dark:text-[#828ca5] uppercase">Status</label>
                    <select value={editForm.status} onChange={e => setEditForm({ ...editForm, status: e.target.value })} className="w-full text-xs font-medium text-slate-700 dark:text-gray-200 border border-slate-200 dark:border-gray-600 rounded-md px-2 py-1.5 focus:border-teal-400 focus:ring-1 focus:ring-teal-400/20 outline-none bg-white dark:bg-[#141722] mt-1">
                  ...
                  <div>
                    <label className="text-[10px] font-semibold text-slate-500 dark:text-[#828ca5] uppercase">Tipo</label>
                    <select value={editForm.type} onChange={e => setEditForm({ ...editForm, type: e.target.value })} className="w-full text-xs font-medium text-slate-700 dark:text-gray-200 border border-slate-200 dark:border-gray-600 rounded-md px-2 py-1.5 focus:border-teal-400 focus:ring-1 focus:ring-teal-400/20 outline-none bg-white dark:bg-[#141722] mt-1">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 bg-slate-50 dark:bg-[#1a1f28] rounded-lg border border-slate-200 dark:border-[#252a3a]">
              <div className="flex items-center gap-1.5 mb-1.5"><CalendarDays size={14} className="text-teal-500 dark:text-teal-400"/><span className="text-[10px] font-semibold text-slate-500 dark:text-[#828ca5] uppercase">Data</span></div>
              {isEditing ? (
                <input type="text" value={editForm.dateDisplay} onChange={e => handleDateMaskedInput(e.target.value)} placeholder="DD/MM/AAAA" maxLength={10} className="w-full text-sm font-semibold text-slate-700 dark:text-gray-200 border border-slate-200 dark:border-gray-600 rounded-md px-2 py-1.5 focus:border-teal-400 focus:ring-1 focus:ring-teal-400/20 outline-none bg-white dark:bg-[#141722]" />
              ) : (
            ...
            <div className="p-3 bg-slate-50 dark:bg-[#1a1f28] rounded-lg border border-slate-200 dark:border-[#252a3a]">
              <div className="flex items-center gap-1.5 mb-1.5"><Clock size={14} className="text-teal-500 dark:text-teal-400"/><span className="text-[10px] font-semibold text-slate-500 dark:text-[#828ca5] uppercase">Horario</span></div>
              {isEditing ? (
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2.5 p-2.5 bg-slate-50 dark:bg-[#1a1f28] rounded-lg border border-slate-200 dark:border-[#252a3a]">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[10px] font-semibold text-slate-500 dark:text-[#828ca5] uppercase">Profissional</p>
                {isEditing ? (
                  <select value={editForm.doctor_id || ''} onChange={e => setEditForm({ ...editForm, doctor_id: Number(e.target.value) })} className="w-full text-sm text-slate-700 dark:text-gray-200 font-medium border border-slate-200 dark:border-gray-600 rounded-md px-2 py-1.5 focus:border-teal-400 focus:ring-1 focus:ring-teal-400/20 outline-none bg-white dark:bg-[#141722]">
            ...
            <div className="flex items-center gap-2.5 p-2.5 bg-slate-50 dark:bg-[#1a1f28] rounded-lg border border-slate-200 dark:border-[#252a3a]">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[10px] font-semibold text-slate-500 dark:text-[#828ca5] uppercase">Responsavel</p>
                {isEditing ? (
                  <input type="text" value={editForm.parent_name} onChange={e => setEditForm({ ...editForm, parent_name: e.target.value })} placeholder="Nome do responsavel" className="w-full text-sm text-slate-700 dark:text-gray-200 font-medium border border-slate-200 dark:border-gray-600 rounded-md px-2 py-1.5 focus:border-teal-400 focus:ring-1 focus:ring-teal-400/20 outline-none bg-white dark:bg-[#141722]" />
                ) : (
            ...
            <div className="flex items-center gap-2.5 p-2.5 bg-slate-50 dark:bg-[#1a1f28] rounded-lg border border-slate-200 dark:border-[#252a3a]">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[10px] font-semibold text-slate-500 dark:text-[#828ca5] uppercase">Telefone</p>
                {isEditing ? (
                  <input type="text" className="w-full text-sm font-medium text-slate-700 dark:text-gray-200 border border-slate-200 dark:border-gray-600 rounded-md px-2 py-1 focus:border-teal-400 focus:ring-1 focus:ring-teal-400/20 outline-none bg-white dark:bg-[#141722]" value={editForm.parent_phone} onChange={(e) => setEditForm({ ...editForm, parent_phone: e.target.value })} placeholder="11999999999" />
                ) : (
          ...
          {selectedAppointment.status !== 'blocked' && (
            <div className="bg-slate-50 dark:bg-[#1a1f28] p-3 rounded-xl border border-slate-200 dark:border-[#252a3a]">
              ...
              {isEditing ? (
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="text" value={editForm.totalAmount} onChange={e => handleMoneyInput('totalAmount', e.target.value)} className="w-full text-sm font-bold border border-slate-200 dark:border-gray-600 rounded px-2 py-1 bg-white dark:bg-[#141722] text-slate-700 dark:text-gray-200 focus:outline-none focus:ring-1 focus:ring-teal-400" />
                  </div>
                  <div>
                  ...
                  <div className="col-span-2 text-right text-xs font-bold text-slate-500 dark:text-[#828ca5] border-t border-slate-200 dark:border-[#252a3a] pt-2 mt-1">
                ...
                <div className="flex justify-between items-center bg-white dark:bg-black/20 p-2.5 rounded-lg border border-slate-100 dark:border-[#252a3a]">
                  ...
                  <div className="text-right pl-4 border-l border-slate-100 dark:border-[#252a3a] ml-4"><p className="text-[10px] text-slate-400 uppercase font-semibold">Falta</p><p className={`text-sm font-black ${remainingDisplay > 0 ? 'text-rose-500' : 'text-slate-400'}`}>R$ {remainingDisplay.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p></div>
                </div>
              )}
            </div>
          )}
          ...
          <div className="p-3 bg-teal-50/30 dark:bg-teal-900/5 rounded-lg border border-teal-200/30 dark:border-teal-800/20">
            ...
            {isEditing ? (
              ...
              <div className="p-2.5 bg-white/60 dark:bg-[#1a1f28]/60 rounded-lg">
                <p className="text-xs text-slate-600 dark:text-[#a0a8be] leading-relaxed">{selectedAppointment.notes || 'Nenhuma observacao registrada.'}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="p-4 bg-slate-50/50 dark:bg-[#0d0f15] border-t border-slate-200 dark:border-[#252a3a] flex gap-2">
          {isEditing ? (
            <>
              <button onClick={() => setIsEditing(false)} className="flex-1 bg-white dark:bg-[#141722] border border-slate-300 dark:border-gray-600 text-slate-600 dark:text-[#a0a8be] py-2 rounded-lg text-xs font-semibold hover:bg-slate-50 dark:hover:bg-white/10 transition-colors">Cancelar</button>
            ...
            <>
              ...
              <button onClick={() => setIsEditing(true)} className="flex-1 bg-white dark:bg-[#141722] border border-slate-300 dark:border-gray-600 text-slate-600 dark:text-[#a0a8be] py-2 rounded-lg text-xs font-semibold hover:bg-slate-50 dark:hover:bg-white/10 transition-colors flex items-center justify-center gap-1.5"><Edit2 size={14}/> Editar</button>
