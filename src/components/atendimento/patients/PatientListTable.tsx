          ...
          <div className="bg-white dark:bg-[#111118] rounded-xl border border-slate-200 dark:border-[#252530] overflow-hidden">
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-gray-800">
                {patients.map(p => (
                  <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors cursor-pointer" onClick={() => router.push(`/atendimento/clients/${p.id}`)}>
                    <td className="px-4 py-3 text-xs text-slate-400 dark:text-[#71717a] font-mono">#{p.id}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center text-xs font-bold text-blue-700 dark:text-blue-300 flex-shrink-0">
