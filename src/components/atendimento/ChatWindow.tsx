...
import ClaraSuggestionToast from '@/components/chat/ClaraSuggestionToast';
import type { SuggestionPhase } from '@/components/chat/ClaraSuggestionToast';
import ClaraSuggestionPanel from '@/components/chat/ClaraSuggestionPanel';
...
export default function AtendimentoChatWindow({ chat }: { chat: Chat | null }) {
  ...
  useEffect(() => {
    ...
    setAiDraftCreatedAt((chat as any)?.ai_draft_created_at ?? null);
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const [aiDraftCreatedAt, setAiDraftCreatedAt] = useState<string | null>((chat as any)?.ai_draft_created_at ?? null);
  ...
  const getDraftShownKey = (chatId: number | string) => `atd_copilot_draft_shown_${chatId}`;
  const getScheduleShownKey = (chatId: number | string) => `atd_copilot_schedule_shown_${chatId}`;
  ...
  useEffect(() => {
    ...
    if (elapsed > DRAFT_EXPIRY_MS) {
      console.log(`[Copiloto Geral] Draft expirado (${Math.round(elapsed / 60000)}min). Limpando.`);
      setAiDraftText(null);
  ...
  // Auto-dismiss: limpa TODOS os drafts quando contexto muda
  ...
  const handleSendFile = useCallback(async (file: File | Blob, caption: string, typeOverride?: string, metadata?: any) => {
    ...
    try {
      ...
      if (isPdfFile && evolutionMediaType === 'document') {
        ...
        if (previewBlob) {
          const previewName = `${realChatId}_${Date.now()}_preview_${cleanName.replace(/\.pdf$/i, '')}.jpg`;
  ...
  if (!chat) {
    return (
      <div className="flex-1 bg-[#F8FAFC] dark:bg-[#0e0e14] flex items-center justify-center text-gray-400 dark:text-gray-600 border-b-[6px] border-blue-500">
        <User size={64} />
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex h-full overflow-hidden bg-[#F8FAFC] dark:bg-[#111118] relative transition-colors duration-300 min-w-0">
