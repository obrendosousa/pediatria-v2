...
export function useChatList(isViewingArchived: boolean, searchTerm: string, options?: UseChatListOptions) {
  ...
  const sortChats = (chatsList: Chat[]): Chat[] => {
    return [...chatsList].sort((a, b) => {
      ...
      const isAiA = a.phone === '00000000001';
      const isAiB = b.phone === '00000000001';
  ...
  const chatsListCacheKey = `atd_chats_list_${isViewingArchived}_${searchTerm || ''}`;
  const tagsCacheKey = 'atd_chats_tags';
  ...
  const fetchChats = async (showLoading = true) => {
    try {
      ...
      if (!isViewingArchived && !searchTerm) {
        const aiChatExists = fetchedData.some(c => c.phone === '00000000001');
        if (!aiChatExists) {
          const { data: existingAi } = await supabase.from('chats').select('*').eq('phone', '00000000001').maybeSingle();
          ...
          if (existingAi) {
            ...
            const { data: newAi } = await supabase.from('chats').insert({
              phone: '00000000001',
              contact_name: '🤖 Agente Clínica Geral',
              is_pinned: true,
              unread_count: 0
            }).select().single();
  ...
  const handleSelectChat = async (chat: Chat) => {
    ...
    if ((chat.unread_count || 0) > 0) {
      await fetch('/api/atendimento/chats/mark-read', {
        method: 'POST',
  ...
  const performAction = async (action: string, chat: Chat) => {
    ...
    if (chat.phone === '00000000001' && (action === 'block' || action === 'delete' || action === 'archive')) {
      alert("Operação não permitida no chat do Agente IA.");
      return false;
    }
  ...
  const handleBulkAction = async (action: 'archive' | 'delete', selectedIds: number[]) => {
    const aiChat = chats.find(c => c.phone === '00000000001');
