...
const STATUS_CONFIG: Record<InvoiceStatus, { label: string; bg: string; text: string }> = {
  ...
  cancelled: { label: 'CANCELADA', bg: 'bg-slate-100 dark:bg-gray-800', text: 'text-slate-500 dark:text-gray-400' },
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 pt-5 pb-4 flex items-center justify-between border-b border-slate-200 dark:border-gray-700">
        <div>
          <h2 className="text-lg font-bold text-slate-800 dark:text-gray-100">Notas Fiscais Eletrônicas</h2>
          <p className="text-xs text-slate-400 dark:text-gray-500 mt-0.5">{totalCount} nota{totalCount !== 1 ? 's' : ''}</p>
        </div>
        <button
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 custom-scrollbar">
        {isLoading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-sm text-slate-400 dark:text-gray-500">Nenhuma nota fiscal encontrada.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="bg-slate-50 dark:bg-[#2a2d36] border-b border-slate-200 dark:border-gray-700">
                  ...
                  <th className="px-4 py-3 text-xs font-extrabold text-slate-500 dark:text-gray-400 uppercase">NF-e</th>
                ...
                {invoices.map(inv => {
                  ...
                  return (
                    <tr key={inv.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                      <td className="px-4 py-3 text-xs text-slate-400 dark:text-gray-500 font-mono">#{inv.id}</td>
                      <td className="px-4 py-3">
                        <p className="font-medium text-slate-700 dark:text-gray-200">{inv.patient_name || '—'}</p>
                      </td>
                      <td className="px-4 py-3 text-right text-slate-600 dark:text-gray-300 font-mono text-xs font-bold">
                      ...
                      <td className="px-4 py-3 text-xs text-slate-600 dark:text-gray-300 font-mono">
                        {inv.nfe_number || '—'}
                      </td>
                      <td className="px-4 py-3">
