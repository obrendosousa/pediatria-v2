  ...
  return (
    <>
              ...
              {groupedRecords[Number(year)].map((record, index) => (
                <div key={record.id} className={`group relative pl-16 ${index === 0 ? 'pt-20' : ''}`}>
                  ...
                  <div className={`absolute left-0 flex flex-col items-center z-40 ${index === 0 ? 'top-20' : 'top-6'}`}>
                    ...
                    <div className={`relative w-9 h-9 rounded-full border-[3px] border-white dark:border-[#0b141a] shadow-lg flex items-center justify-center transition-all duration-200 group-hover:scale-110 ${record.status === 'signed'
      ...
      <AttendanceDetailModal
        ...
        externalPatientData={patientData}
      />
    </>
  );
}