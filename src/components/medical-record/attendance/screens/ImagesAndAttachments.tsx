      ...
      >
        {isImage(file.file_type) ? (
          ...
          <div className="flex flex-col items-center gap-2 p-4">
            ...
            <span className="text-xs text-slate-500 dark:text-[#a1a1aa] text-center break-all line-clamp-2">
              {file.file_name}
            </span>
          </div>
        )}
...
export function ImagesAndAttachments({ patientId, patientData, appointmentId, medicalRecordId }: AttendanceScreenProps) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* ── Cabeçalho ──────────────────────────────────────────────────── */}
      <div className="px-6 pt-5 pb-4 flex items-start justify-between border-b border-slate-200 dark:border-[#3d3d48]">
        <div>
          <h2 className="text-lg font-bold text-slate-800 dark:text-[#fafafa]">Imagens e Anexos</h2>
          <p className="text-xs text-slate-400 dark:text-[#71717a] mt-0.5">{today}</p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 custom-scrollbar space-y-6">
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center py-10 gap-4 text-center">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400 dark:text-[#71717a]">ou</p>
            </div>
              ...
              {pendingFiles.map((f, i) => (
                <div key={i} className="relative group rounded-xl border-2 border-dashed border-amber-300 dark:border-amber-700 overflow-hidden bg-amber-50 dark:bg-amber-900/10">
                  ...
                  <div className="px-2 py-1 bg-white dark:bg-[#08080b]">
                    ...
                    <p className="text-xs text-slate-400 dark:text-[#71717a]">{formatFileSize(f.size)}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {isLoading ? (
          ...
          <div>
            <h3 className="text-sm font-semibold text-slate-600 dark:text-[#d4d4d8] mb-3 flex items-center justify-between">
