...
// ─── Geração de PDF ───────────────────────────────────────────────────────────
export type PrescriptionDocType = 'medications' | 'exams' | 'vaccines';

const DOC_TITLES: Record<PrescriptionDocType, string> = {
  medications: 'Receita Médica',
  exams: 'Solicitação de Exames',
  vaccines: 'Cartão de Vacinas',
};

export function generatePrescriptionHTML(draft: DraftPrescription, patientData: any, docType?: PrescriptionDocType): string {
  ...
  const showMeds = !docType || docType === 'medications';
  const showExams = !docType || docType === 'exams';
  const showVaccines = !docType || docType === 'vaccines';

  const title = docType ? DOC_TITLES[docType] : 'Receita Médica';
...
function PrescriptionCard({
  prescription, onRenew, onPrintType, onDelete,
}: {
  prescription: Prescription;
  onRenew: () => void;
  onPrintType: (type: PrescriptionDocType) => void;
  onDelete: () => void;
}) {
  ...
  return (
    <div className="bg-white dark:bg-[#1e2028] border border-slate-200 dark:border-gray-700 rounded-lg p-4">
      ...
      {/* Botões de impressão por tipo */}
      <div className="mt-3 flex flex-wrap gap-1.5">
        {meds.length > 0 && (
          <button type="button" onClick={() => onPrintType('medications')} title="Imprimir Medicamentos"
            className="flex items-center gap-1 px-2 py-1 text-[10px] font-semibold text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-700 rounded hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors">
            <Pill className="w-3 h-3" /> Medicamentos
          </button>
        )}
        {exams.length > 0 && (
          <button type="button" onClick={() => onPrintType('exams')} title="Imprimir Exames"
            className="flex items-center gap-1 px-2 py-1 text-[10px] font-semibold text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-700 rounded hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-colors">
            <Microscope className="w-3 h-3" /> Exames
          </button>
        )}
        {vaccines.length > 0 && (
          <button type="button" onClick={() => onPrintType('vaccines')} title="Imprimir Vacinas"
            className="flex items-center gap-1 px-2 py-1 text-[10px] font-semibold text-purple-600 dark:text-purple-400 border border-purple-200 dark:border-purple-700 rounded hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors">
            <Syringe className="w-3 h-3" /> Vacinas
          </button>
        )}
      </div>

      <button type="button" onClick={onRenew} className="mt-2 w-full flex items-center justify-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-700 rounded-md hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors">
    ...
    return (
      <div className="p-4 pb-6">
        ...
        <div className="bg-white dark:bg-[#1e2028] border border-slate-200 dark:border-gray-700 rounded-lg p-5 mb-4">
          ...
          <p className="text-sm text-slate-500 dark:text-gray-400 mb-4">Imprima os documentos separadamente:</p>

          {meds.length > 0 && (
            <div className="flex items-center justify-between gap-2 mb-2 p-3 border border-slate-200 dark:border-gray-700 rounded-lg">
              <div className="flex items-center gap-2">
                <Pill className="w-4 h-4 text-blue-500" />
                <span className="text-sm text-slate-700 dark:text-gray-200">
                  Receituário ({meds.length} medicamento{meds.length > 1 ? 's' : ''})
                </span>
              </div>
              <button type="button"
                onClick={() => printPrescription({ medications: meds, exams, vaccines }, patientData, 'medications')}
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-blue-600 dark:text-blue-400 border border-blue-300 dark:border-blue-700 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors">
          ...
          {exams.length > 0 && (
            <div className="flex items-center justify-between gap-2 mb-2 p-3 border border-slate-200 dark:border-gray-700 rounded-lg">
              ...
              <button type="button"
                onClick={() => printPrescription({ medications: meds, exams, vaccines }, patientData, 'exams')}
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-emerald-600 dark:text-emerald-400 border border-emerald-300 dark:border-emerald-700 rounded-lg hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-colors">
                <Printer className="w-3.5 h-3.5" /> Imprimir
              </button>
            </div>
          )}
          {vaccines.length > 0 && (
            <div className="flex items-center justify-between gap-2 mb-2 p-3 border border-slate-200 dark:border-gray-700 rounded-lg">
              ...
              <button type="button"
                onClick={() => printPrescription({ medications: meds, exams, vaccines }, patientData, 'vaccines')}
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-purple-600 dark:text-purple-400 border border-purple-300 dark:border-purple-700 rounded-lg hover:bg-purple-50 dark:hover:bg-purple-900/20 transition-colors">
                <Printer className="w-3.5 h-3.5" /> Imprimir
              </button>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {(showAllHistory ? prescriptions : prescriptions.slice(0, 3)).map((p) => (
              <PrescriptionCard key={p.id} prescription={p}
                ...
                onPrintType={(type) => printPrescription({
