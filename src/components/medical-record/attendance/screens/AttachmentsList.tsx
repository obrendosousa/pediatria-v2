      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 custom-scrollbar space-y-5">
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center py-8 gap-3 text-center">
            ...
            <div>
              <p className="text-sm text-slate-600 dark:text-[#d4d4d8]">Arraste arquivos para cá ou</p>
              <button
                type="button"
                onClick={() => fileInputRef.current?.click()}
            ...
            <div className="space-y-1">
              {pendingFiles.map((f, i) => (
                <div key={i} className="flex items-center gap-3 px-4 py-2 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/30 rounded-lg">
                  ...
                  <span className="text-xs text-slate-400 dark:text-[#71717a]">{formatFileSize(f.size)}</span>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-8">
            ...
            <p className="text-sm text-slate-400 dark:text-[#71717a]">Nenhum arquivo encontrado para &quot;{searchTerm}&quot;.</p>
          </div>
        ) : (
          !pendingFiles.length && (
            <div className="text-center py-16">
              <Paperclip className="w-12 h-12 text-slate-300 dark:text-gray-600 mx-auto mb-3" />
              <p className="text-sm text-slate-400 dark:text-[#71717a]">Nenhum anexo enviado para este paciente.</p>
            </div>
          )
        )}
      </div>
