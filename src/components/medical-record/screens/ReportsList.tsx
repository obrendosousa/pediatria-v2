...
import { useReports, type MedicalReport } from '@/hooks/atendimento/useReports';
...
export function ReportsList({ patientId, patientData, appointmentId }: AttendanceScreenProps) {
  ...
  const handleEdit = (report: MedicalReport) => {
    ...
    setDigitalSignature(report.digital_signature);
    setShowDate(report.show_date ?? true);
    setReportDate(report.report_date || new Date().toLocaleDateString('en-CA'));
  ...
  const handleSave = async () => {
    if (!content.trim()) {
      toast.error('O conteúdo do laudo é obrigatório.');
      return;
    }
    setSaving(true);
    try {
      const payload = {
        ...
        show_date: showDate,
        report_date: showDate ? reportDate : null,
  ...
  const handlePrint = async (report: MedicalReport) => {
    const { printWithLetterhead } = await import('@/lib/letterhead');
    const doctorName = report.doctor_id ? (doctors[report.doctor_id] || '—') : '';
    await printWithLetterhead(`
      <h1>${report.title || 'Laudo Médico'}</h1>
      <div class="meta">
        ${report.show_date !== false && report.report_date ? `<div>Data: ${new Date(report.report_date + 'T12:00:00').toLocaleDateString('pt-BR')}</div>` : ''}
        ${doctorName ? `<div>Profissional: ${doctorName}</div>` : ''}
      </div>
      <hr/>
      <div class="content">${report.content || ''}</div>
      ${report.digital_signature ? '<p style="margin-top:40px;font-weight:bold">Assinado digitalmente</p>' : ''}
    `, report.title || 'Laudo Médico');
  };
    ...
    return (
      <div className="p-6 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-bold text-slate-800 dark:text-[#fafafa]">{selected.title || 'Laudo'}</h2>
        ...
        <div className="bg-white dark:bg-[#08080b] rounded-xl border border-slate-200 dark:border-[#3d3d48] p-6 space-y-3">
          <div className="flex gap-6 text-sm text-slate-500 dark:text-[#a1a1aa]">
            {selected.show_date && selected.report_date && (
              <span>Data: <strong className="text-slate-700 dark:text-gray-200">{new Date(selected.report_date + 'T12:00:00').toLocaleDateString('pt-BR')}</strong></span>
            )}
  ...
  if (mode === 'create' || mode === 'edit') {
    return (
      <ClinicalDocumentEditor
        mode={mode}
        createLabel="Novo Laudo"
        editLabel="Editar Laudo"
        onBack={handleBack}
        showTitle
        title={title}
        onTitleChange={setTitle}
        titlePlaceholder="Ex: Laudo de avaliação"
        ...
        dateLabel="Data do laudo"
        content={content}
        onContentChange={setContent}
        contentPlaceholder="Digite o conteúdo do laudo..."
        templates={templates}
        onSelectTemplate={handleSelectTemplate}
        templatePanelTitle="Modelos de laudo"
        saving={saving}
        onSave={handleSave}
        saveIcon={ScrollText}
      />
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-slate-800 dark:text-[#fafafa]">Laudos</h2>
        <button
        ...
        >
          <Plus className="w-4 h-4" /> ADICIONAR LAUDO
        </button>
      </div>

      {reports.length === 0 ? (
        <div className="text-center py-16">
          <ScrollText className="w-12 h-12 text-slate-300 dark:text-gray-600 mx-auto mb-3" />
          <p className="text-sm text-slate-400 dark:text-[#71717a]">Nenhum laudo registrado.</p>
        </div>
      ) : (
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-gray-800">
              {reports.map(report => (
                <tr key={report.id} className="hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-4 py-3 text-slate-600 dark:text-[#d4d4d8]">
                    {report.report_date ? new Date(report.report_date + 'T12:00:00').toLocaleDateString('pt-BR') : '—'}
                  </td>
                  ...
                  <td className="px-4 py-3 text-slate-600 dark:text-[#d4d4d8]">
                    {report.doctor_id ? (doctors[report.doctor_id] || '—') : '—'}
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => handleView(report)} title="Visualizar" className="p-1.5 rounded-lg text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors">
                        <Eye className="w-4 h-4" />
                      </button>
                      <button onClick={() => handleEdit(report)} title="Editar" className="p-1.5 rounded-lg text-slate-400 hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-colors">
                        <Pencil className="w-4 h-4" />
                      </button>
                      <button onClick={() => setConfirmDeleteId(report.id)} title="Excluir" className="p-1.5 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
                        <Trash2 className="w-4 h-4" />
                      </button>
                      <button onClick={() => handlePrint(report)} title="Imprimir" className="p-1.5 rounded-lg text-slate-400 hover:text-slate-600 hover:bg-slate-100 dark:hover:bg-white/10 transition-colors">
      ...
      <ConfirmModal
        ...
        title="Excluir laudo"
        message="Tem certeza que deseja excluir este laudo? Esta ação não pode ser desfeita."
        confirmText="Excluir"
        type="danger"
      />
    </div>
