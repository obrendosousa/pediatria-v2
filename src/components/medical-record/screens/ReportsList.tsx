...
export function ReportsList({ patientId, patientData, appointmentId }: AttendanceScreenProps) {
  ...
  const handlePrint = (report: MedicalReport) => {
    ...
    win.document.write(`
      <html>
      <head>
        <title>${report.title || 'Laudo Médico'}</title>
    ...
    return (
      <div className="p-6 space-y-4">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-lg font-bold text-slate-800 dark:text-[#e8ecf4]">{selected.title || 'Laudo'}</h2>
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-slate-800 dark:text-[#e8ecf4]">Laudos</h2>
        <button
      ...
      {reports.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-sm text-slate-400 dark:text-[#565d73]">Nenhum laudo registrado.</p>
        </div>
      ) : (
