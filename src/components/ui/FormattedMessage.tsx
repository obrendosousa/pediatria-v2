  ...
  const content = useMemo(() => {
    ...
    const processInlineFormatting = (line: string, lineIdx: number): React.ReactNode[] => {
      ...
      return monoBlockParts.flatMap((part, mbIdx) => {
        ...
        if (isMonoBlock) {
          return [
            <code key={`${lineIdx}-mb-${mbIdx}`} className="bg-gray-200 dark:bg-[#1e2334] px-1.5 py-0.5 rounded text-sm font-mono">
              {part}
            </code>
          ];
        }
        ...
        return inlineCodeParts.flatMap((icPart, icIdx) => {
          ...
          if (isInlineCode) {
            return [
              <code key={`${lineIdx}-ic-${mbIdx}-${icIdx}`} className="bg-gray-200 dark:bg-[#1e2334] px-1 py-0.5 rounded text-sm font-mono">
                {icPart}
              </code>
            ];
          }
