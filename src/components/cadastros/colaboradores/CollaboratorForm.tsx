...
import type { Collaborator, CollaboratorRole } from '@/types/cadastros';

// --- Constantes ---

const ROLE_OPTIONS: { value: CollaboratorRole; label: string }[] = [
  { value: 'administrator', label: 'Administrador da clínica' },
  { value: 'administrative_assistant', label: 'Auxiliar administrativo' },
  { value: 'other', label: 'Outro tipo de colaborador' },
...
const EMPTY_FORM: CollaboratorFormData = {
  ...
  role: '', schedule_access: 'view_appointment',
  is_admin: false,
  attachments: [],
  notes: '',
  send_info: false,
...
function collaboratorToForm(c: Collaborator): CollaboratorFormData {
  return {
    name: c.name,
    sex: c.sex || '',
    ...
    whatsapp: c.whatsapp || '',
    role: c.role,
    schedule_access: c.schedule_access,
    is_admin: c.is_admin,
    attachments: [],
    notes: c.notes || '',
    send_info: false,
...
export default function CollaboratorForm({ initialData, onSubmit, title, subtitle }: CollaboratorFormProps) {
  const router = useRouter();
  const { toast } = useToast();

  const [form, setForm] = useState<CollaboratorFormData>(
    initialData ? collaboratorToForm(initialData) : { ...EMPTY_FORM },
  );
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [saving, setSaving] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const update = useCallback(<K extends keyof CollaboratorFormData>(key: K, value: CollaboratorFormData[K]) => {
  ...
  const validate = (): boolean => {
    ...
    if (!form.role) errs.role = 'Cargo é obrigatório.';
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <section className="bg-white dark:bg-[#08080b] rounded-xl border border-slate-200 dark:border-[#3d3d48] p-6 space-y-5">
            ...
            <div className="grid grid-cols-12 gap-5">
              {/* Cargo */}
              <div className="col-span-12 md:col-span-6">
                <label className={labelClass}>Cargo <RequiredBadge /></label>
                <div className="relative">
                  <select
                    value={form.role}
                    onChange={e => update('role', e.target.value)}
                    className={`${selectClass} ${errors.role ? 'border-red-300 dark:border-red-700' : ''}`}
                  >
                    <option value="">Selecione</option>
          ...
          <div className="bg-white dark:bg-[#08080b] rounded-xl border border-slate-200 dark:border-[#3d3d48] p-6">
            <label className="flex items-center gap-3 cursor-pointer">
              <input
                type="checkbox"
                checked={form.send_info}
                onChange={e => update('send_info', e.target.checked)}
                className="w-4 h-4 rounded border-slate-300 text-teal-600 focus:ring-teal-500"
              />
              <span className="text-sm text-slate-700 dark:text-gray-200 font-medium">
                Enviar informações (enviar convite por e-mail)
              </span>
            </label>
          </div>
        </div>
