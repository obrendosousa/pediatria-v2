...
export function useChatList(isViewingArchived: boolean, searchTerm: string, options?: UseChatListOptions) {
  ...
  const fetchChats = useCallback(async (showLoading = true) => {
    try {
      ...
      set(`chats_list_${isViewingArchived}_${searchTerm || ''}`, sortedChats, TTL_CHATS_LIST_MS);
    } catch (e) {
      console.error(e);
    } finally {
      setIsLoading(false);
  ...
  useRealtimeSubscription({
    client: supabase,
    channelName: 'public:chat_messages_for_list',
    schema: 'public',
    table: 'chat_messages',
    event: 'INSERT',
