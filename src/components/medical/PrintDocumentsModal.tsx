      ...
      <div className="bg-white dark:bg-[#08080b] w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 px-6 py-4 border-b border-slate-200 dark:border-[#3d3d48] flex justify-between items-center">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-[#a1a1aa] mt-0.5">{patientName}</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4 custom-scrollbar">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-sm text-slate-500 dark:text-[#a1a1aa]">Nenhum documento gerado nesta consulta.</p>
              <p className="text-xs text-slate-400 dark:text-[#71717a] mt-1">Receitas, exames e atestados aparecem aqui.</p>
            </div>
          ) : (
            <>
              ...
              {prescriptions.length > 0 && prescriptions.map((presc: any, idx: number) => {
                ...
                return (
                  <div key={presc.id || idx}>
                    <h3 className="text-xs font-bold text-slate-500 dark:text-[#a1a1aa] uppercase tracking-wider mb-2 flex items-center gap-1.5">
                          ...
                          <div key={type} className="flex items-center justify-between bg-slate-50 dark:bg-[#1c1c21] rounded-lg border border-slate-200 dark:border-[#3d3d48] p-3">
                            <div className="flex items-center gap-2 min-w-0 flex-1 mr-3">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-slate-500 dark:text-[#a1a1aa]">{count} ite{count > 1 ? 'ns' : 'm'}</p>
                              </div>
                            </div>
