...
export default function Sidebar({ onSelectChat, selectedChatId }: SidebarProps) {
  ...
  return (
    <>
      <div className="w-[320px] min-w-[260px] md:w-[380px] lg:w-[400px] shrink-0 flex flex-col border-r border-gray-200 dark:border-[#1e2334] bg-white dark:bg-[#0d0f15] h-full relative z-10 transition-colors duration-300">
        ...
        {/* 1. MODO ARQUIVADOS */}
        {isViewingArchived ? (
          <div className="flex flex-col animate-in fade-in duration-200 bg-[#f0f2f5] dark:bg-[#141722]">
            <div className="h-[60px] flex items-center px-4 gap-4 border-b border-gray-200/50 dark:border-[#252a3a]">
              <button
                onClick={() => setIsViewingArchived(false)}
                className="hover:bg-gray-200 dark:hover:bg-white/10 rounded-full p-2 transition-colors -ml-2 text-[#54656f] dark:text-[#a0a8be]"
              >
                <ArrowLeft size={20} />
              </button>
              <h2 className="text-[19px] font-medium text-[#111b21] dark:text-[#e8ecf4]">Arquivadas</h2>
            </div>
            <div className="py-4 px-8 text-center bg-gray-50/50 dark:bg-transparent border-b border-gray-100 dark:border-[#1e2334]">
              <p className="text-[13px] text-[#54656f] dark:text-[#828ca5] leading-relaxed">
          ...
          <div className="h-[60px] bg-primary/15 dark:bg-primary/20 flex items-center justify-between px-4 shrink-0 border-b border-primary/20 animate-in fade-in slide-in-from-top-2">
            <div className="flex items-center gap-3">
              ...
              <span className="font-bold text-primary-foreground/90 dark:text-[#e8ecf4] text-sm">
          ...
          <>
            <div className="h-[60px] bg-[#f0f2f5] dark:bg-[#141722] flex items-center justify-between px-4 shrink-0 border-b border-gray-200/60 dark:border-[#252a3a]">
              <div
              ...
              <div className="flex gap-1 text-[#54656f] dark:text-[#a0a8be]">
                <button
                  onClick={() => setIsNewChatModalOpen(true)}
                  className="hover:bg-gray-200/70 dark:hover:bg-white/10 rounded-lg p-2 transition-colors cursor-pointer"
                  title="Nova Conversa"
                ...
                <button
                  ...
                  className={`relative hover:bg-gray-200/70 dark:hover:bg-white/10 rounded-lg p-2 transition-colors cursor-pointer ${isPaused ? 'text-red-500 dark:text-red-400' : ''
                    }`}
                ...
                >
                  ...
                  {isPaused && (
                    <span className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-[#f0f2f5] dark:border-[#2a2d36] animate-pulse" />
                  )}
                </button>
                ...
                <div className="relative" ref={headerMenuRef}>
                  <button
                    ...
                    className={`hover:bg-gray-200/70 dark:hover:bg-white/10 rounded-lg p-2 transition-colors cursor-pointer ${isHeaderMenuOpen ? 'bg-gray-200/70 dark:bg-white/10' : ''}`}
                    aria-label="Menu"
                  >
                  ...
                  {isHeaderMenuOpen && (
                    <div className="absolute right-0 top-12 w-56 bg-white dark:bg-[#141722] rounded-xl shadow-[0_4px_24px_rgba(0,0,0,0.12)] py-1.5 z-[60] border border-gray-100 dark:border-[#252a3a] animate-in fade-in zoom-in-95 duration-200 origin-top-right transition-all">
                      <button
                      ...
                      >
                        <Tag size={16} className="text-[#54656f] dark:text-[#828ca5]" /> Gerenciar Etiquetas
                      </button>
                      <button className="w-full text-left px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-white/5 flex items-center gap-3 text-[14px] text-[#3b4a54] dark:text-gray-200 cursor-pointer transition-colors">
                        <Settings size={16} className="text-[#54656f] dark:text-[#828ca5]" /> Configurações
                      </button>
                    </div>
                  )}
                </div>
            ...
            <div className="px-3 py-2 border-b border-gray-100 dark:border-[#1e2334] bg-white dark:bg-[#0d0f15] relative transition-colors duration-300">
              <div className="bg-[#f0f2f5] dark:bg-[#141722] rounded-lg flex items-center px-3.5 py-1.5 h-[36px] transition-colors duration-300 focus-within:ring-2 focus-within:ring-green-300/50 dark:focus-within:ring-green-700/50">
                <Search size={16} className="text-[#54656f] dark:text-[#828ca5] mr-3 shrink-0" />
                <input
                  type="text"
                  placeholder="Pesquisar conversas..."
                  className="bg-transparent outline-none text-[13.5px] w-full placeholder-[#54656f]/70 dark:placeholder-gray-500 text-[#3b4a54] dark:text-gray-200"
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                />
                {searchTerm && (
                  <button onClick={() => setSearchTerm('')} className="text-[#54656f] dark:text-[#828ca5] hover:text-[#3b4a54] dark:hover:text-gray-200 p-0.5 cursor-pointer transition-colors">
                    <X size={14} />
                  </button>
                )}
              </div>
              <button className="absolute right-5 top-1/2 -translate-y-1/2 text-[#54656f] dark:text-[#828ca5] hover:text-[#3b4a54] dark:hover:text-gray-200 cursor-pointer transition-colors" aria-label="Filtrar">
                <Filter size={16} />
              </button>
            </div>
          </>
        ...
        {/* --- LISTAGEM --- */}
        <div className="flex-1 overflow-y-auto scrollbar-thin hover:scrollbar-thumb-gray-300 dark:hover:scrollbar-thumb-gray-600 bg-white dark:bg-[#0d0f15] transition-colors duration-300">

          {/* BOTÕES DE NAVEGAÇÃO SUPERIORES */}
          {!isViewingArchived && !isSelectionMode && !searchTerm && (
            <div
              onClick={() => setIsViewingArchived(true)}
              className="flex items-center px-4 py-2.5 cursor-pointer hover:bg-gray-50 dark:hover:bg-white/5 text-[#111b21] dark:text-gray-200 border-b border-gray-100 dark:border-[#1e2334] transition-colors duration-200"
            >
          ...
          {isLoading && (!chats || chats.length === 0) ? (
            ...
            (chats || []).length > 0 ? (
              ...
              <div className="p-10 text-center flex flex-col items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-500 dark:text-[#828ca5]">
                    {isViewingArchived ? 'Nenhuma conversa arquivada' : searchTerm ? 'Nenhum resultado' : 'Nenhuma conversa'}
                  </p>
                  {searchTerm && (
                    <p className="text-xs text-gray-400 dark:text-[#565d73] mt-1">Tente outro termo de busca</p>
                  )}
                </div>
              </div>
            )
