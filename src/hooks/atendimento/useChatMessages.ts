...
export function useChatMessages(activeChat: Chat | null, options?: UseChatMessagesOptions) {
  ...
  useEffect(() => {
    ...
    const key = `atd_chat_messages_${ownerChatId}`;
  ...
  const sendMessage = async (text: string, options?: SendMessageOptions) => {
    ...
    try {
      ...
      if (String(activeChat.id).startsWith('new_')) {
        ...
        if (updatedMsgs) {
          ...
          const toCache = updatedMsgs.length > MAX_CACHED_MESSAGES_PER_CHAT
            ...
            : updatedMsgs;
          set(`atd_chat_messages_${realChatId}`, toCache, TTL_MESSAGES_MS);
      ...
      // Dispara API do WhatsApp em background
      if (realMsg) {
        const cleanPhone = activeChat.phone.replace(/\D/g, '');
        const remoteJid = `${cleanPhone}@s.whatsapp.net`;
        fetch('/api/atendimento/whatsapp/send', {
          method: 'POST',
  ...
  const editMessage = async (msg: Message, newText: string) => {
    ...
    try {
      const response = await fetch('/api/atendimento/whatsapp/edit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          messageId: msg.id,
  ...
  const reactToMessage = useCallback(async (msg: Message, emoji: string) => {
    ...
    const response = await fetch('/api/atendimento/whatsapp/reaction', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        messageWppId: wppId,
  ...
  const deleteMessage = async (msg: Message, deleteForEveryone: boolean) => {
    ...
    try {
      ...
      const res = await fetch('/api/atendimento/whatsapp/delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
