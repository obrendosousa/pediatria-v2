...
export function useCertificateTemplates() {
  ...
  const createTemplate = useCallback(async (input: CertificateTemplateInput) => {
    setSaving(true);
    try {
      const { data, error } = await supabase
        .from('certificate_templates')
  ...
  const updateTemplate = useCallback(async (id: string, input: Partial<CertificateTemplateInput>) => {
    setSaving(true);
    try {
      const { data, error } = await supabase
        .from('certificate_templates')
