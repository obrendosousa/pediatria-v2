// ═══════════════════════════════════════════════════════════════════════════
// Classification Engine — módulo reutilizável de classificação de chats.
// Extraído do raw_data_analyzer.ts para uso compartilhado entre:
//   - analyze_raw_conversations (tool interativa)
//   - chatClassificationCron (classificação automática a cada 15min)
//   - dailyKpiCron (agregação diária de KPIs)
// ═══════════════════════════════════════════════════════════════════════════
...
// ─────────────────────────────────────────────────────────────────────────────
// SCHEMA DE CLASSIFICAÇÃO (Gemini structured output)
// ─────────────────────────────────────────────────────────────────────────────
...
const classificationSchema = z.object({
  classificacoes: z.array(z.object({
    ...
    objecao_principal: z.string().describe("Objeção principal identificada (vazio se não houver)"),
    ...
    nota_atendimento: z.number().min(0).max(10).describe("Nota de 0-10 para qualidade do atendimento da clínica"),
  })),
});

// ─────────────────────────────────────────────────────────────────────────────
// CLASSIFICAÇÃO DE UM BATCH
// ─────────────────────────────────────────────────────────────────────────────

const CLASSIFICATION_PROMPT = `Você é um analista de atendimento de clínica pediátrica. Classifique CADA conversa.
...
- objecao_distancia: desistiu por distância/logística
- objecao_especialidade: procurava especialidade não disponível
- urgencia_atendida: caso urgente que foi resolvido
- urgencia_nao_atendida: caso urgente sem resolução
- ghosting: paciente sumiu após interação inicial
- informacao_apenas: só perguntou valor/horário sem intenção clara
- encaminhamento_externo: encaminhado para outro local/serviço
- em_andamento: conversa ainda em curso, sem desfecho claro
- sem_resposta_clinica: clínica não respondeu o paciente

REGRAS:
1. Retorne EXATAMENTE uma classificação por conversa
2. Use o chat_id exato fornecido
3. Baseie-se APENAS no texto — NÃO invente
...
7. nota_atendimento: 0 = péssimo (sem resposta), 5 = mediano, 10 = excelente
8. objecao_principal: a objeção mais forte; vazio se não houver objeções`;
...
// ─────────────────────────────────────────────────────────────────────────────
// FUNÇÃO PRINCIPAL: classifyChats
// ─────────────────────────────────────────────────────────────────────────────
 ...
 * 3. Chama gemini-3-flash-preview para cada batch
...
export async function classifyChats(
  ...
  console.log(`[classification_engine] Concluído: ${classifications.length} chats classificados`);
