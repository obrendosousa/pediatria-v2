...
import { clinicaGeralTools, setCurrentTemporalAnchor } from "./tools";
...
import { buildClinicaGeralSystemPrompt, CLINICA_GERAL_SYSTEM_PROMPT } from "./system_prompt";
import { CLINICA_GERAL_COMPANY } from "./company";
import { CLINICA_GERAL_RULES } from "./rules";

// Shared Clara 2.0 imports
import type { TemporalAnchor } from "@/ai/clara/temporal_anchor";
import { resolveTemporalAnchor } from "@/ai/clara/temporal_anchor";
...
import { compactMessages } from "@/ai/clara/context_compactor";
import { extractSpotCheckData, spotCheckCitations, type SpotCheckResult } from "@/ai/clara/spot_check_verifier";
...
// ─────────────────────────────────────────────────────────────────────────────
// ESTADO PRINCIPAL DO CLÍNICA GERAL AGENT
// ─────────────────────────────────────────────────────────────────────────────

export interface ClinicaGeralState {
...
async function loadDynamicPromptParts(): Promise<{
  ...
  try {
    ...
    if (error || !data || data.length === 0) {
      return { company: CLINICA_GERAL_COMPANY, custom_rules: CLINICA_GERAL_RULES, voice_rules: "" };
    }
    ...
    return {
      company: map.company ?? CLINICA_GERAL_COMPANY,
      custom_rules: map.rules ?? CLINICA_GERAL_RULES,
...
const clinicaGeralWorkflow = new StateGraph<ClinicaGeralState>({
  channels: {
    messages: {
    ...
    current_user_role: {
      reducer: (_x: ClinicaGeralState["current_user_role"], y: ClinicaGeralState["current_user_role"]) => y ?? _x ?? "patient",
...
// ─────────────────────────────────────────────────────────────────────────────
// NODE 0: load_context_node (roda ANTES do classify)
// Resolve temporal anchor, carrega DB stats e Auto-RAG em paralelo.
// ─────────────────────────────────────────────────────────────────────────────

clinicaGeralWorkflow.addNode("load_context_node", async (state: ClinicaGeralState) => {
...
clinicaGeralWorkflow.addNode("classify_node", async (state: ClinicaGeralState) => {
  const lastMessage = state.messages[state.messages.length - 1];
  const userText =
    typeof lastMessage?.content === "string"
      ? lastMessage.content
  ...
  const CLASSIFIER_SYSTEM = `Você é um roteador de intenções para o Agente Clínica, assistente de clínica médica geral.

Hoje: ${today}

REGRA DE DECISÃO:

"simple" = O agente responde com suas ferramentas diretas (SQL, analyze_raw_conversations, busca).
...
// ─────────────────────────────────────────────────────────────────────────────
// NODE 2: simple_agent (usa buildClinicaGeralSystemPrompt)
// ─────────────────────────────────────────────────────────────────────────────

clinicaGeralWorkflow.addNode("simple_agent", async (state: ClinicaGeralState) => {
  ...
  // Usa buildClinicaGeralSystemPrompt com temporal anchor, DB stats e memória
  const systemPrompt = buildClinicaGeralSystemPrompt({
    company,
    rules: custom_rules,
...
clinicaGeralWorkflow.addNode("tools", new ToolNode(simpleAgentTools));

// ─────────────────────────────────────────────────────────────────────────────
// NODE 2.5: spot_check_node (verifica citações após tool calls)
// ─────────────────────────────────────────────────────────────────────────────

clinicaGeralWorkflow.addNode("spot_check_node", async (state: ClinicaGeralState) => {
  const lastMessage = state.messages[state.messages.length - 1];
...
clinicaGeralWorkflow.addNode("write_research_brief_node", async (state: ClinicaGeralState) => {
  const lastMessage = state.messages[state.messages.length - 1];
  const userText =
    typeof lastMessage?.content === "string"
  ...
  // Inclui temporal anchor no brief
  ...
  const BRIEF_SYSTEM = `Você é o Estrategista de Pesquisa do Agente Clínica. Transforme a solicitação em um plano de pesquisa estruturado.
...
clinicaGeralWorkflow.addNode("research_supervisor_node", async (state: ClinicaGeralState) => {
  const today = new Date().toISOString().slice(0, 10);
  ...
  if (state.supervisor_iteration === 0) {
    try {
      ...
      const message = e instanceof Error ? e.message : String(e);
      console.warn("[Supervisor] Falha ao coletar contexto:", message);
    }
  }
  ...
  // Inclui temporal anchor
  const temporalInfo = state.temporal_anchor
    ? `\nPERÍODO: ${state.temporal_anchor.period_label}\nSQL start: ${state.temporal_anchor.sql_start}\nSQL end: ${state.temporal_anchor.sql_end}`
    : "";

  const SUPERVISOR_SYSTEM = `Você é o Supervisor de Pesquisa do Agente Clínica.
...
6. PRIORIZE: funil de conversão (new→won), pacientes perdidos (lost), motivos de perda, oportunidades financeiras`;
  ...
  try {
    ...
    const message = e instanceof Error ? e.message : String(e);
    console.error("[Supervisor] Erro. Forçando pesquisa.", message);
    decision = {
...
clinicaGeralWorkflow.addNode("final_report_node", async (state: ClinicaGeralState) => {
...
7. Se não há dados financeiros exatos, ESTIME com base nos chats perdidos × ticket médio da clínica médica geral.
...
${rawNotesText || "[Nenhum dado encontrado.]"}`;
  ...
  if (shouldAutoSave) {
    try {
      ...
      const message = e instanceof Error ? e.message : String(e);
      console.error("[ClinicaGeral] Exceção ao salvar relatório:", message);
    }
  }
...
clinicaGeralWorkflow.addEdge(START, "load_context_node");
clinicaGeralWorkflow.addEdge("load_context_node", "classify_node");
clinicaGeralWorkflow.addConditionalEdges("classify_node", (state: ClinicaGeralState) => {
  return state.is_deep_research ? "write_research_brief_node" : "simple_agent";
});
clinicaGeralWorkflow.addConditionalEdges("simple_agent", (state: ClinicaGeralState) => {
  if ((state.tool_call_count ?? 0) >= MAX_TOOL_CALL_ITERATIONS) return END;
  return toolsCondition(state);
});
clinicaGeralWorkflow.addEdge("tools", "spot_check_node");
clinicaGeralWorkflow.addEdge("spot_check_node", "simple_agent");
clinicaGeralWorkflow.addConditionalEdges("write_research_brief_node", (state: ClinicaGeralState) => {
  return state.research_complete ? END : "research_supervisor_node";
});
clinicaGeralWorkflow.addConditionalEdges("research_supervisor_node", (state: ClinicaGeralState) => {
...
clinicaGeralWorkflow.addEdge("final_report_node", END);

export const clinicaGeralGraph = clinicaGeralWorkflow.compile({
