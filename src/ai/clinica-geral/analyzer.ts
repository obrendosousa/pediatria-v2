...
Se nao houver insight generalizavel, retorne null.`,
        ...
        action: {
            type: Type.OBJECT,
            description: "A ação imediata que o agente deve tomar com base na última mensagem.",
            properties: {
                type: {
                    type: Type.STRING,
...
export async function analyzeChatInteraction(chatId: number) {
    try {
        ...
        const history = messages.reverse().map((m: Record<string, any>) => {
            ...
            const label = senderStr === "AI_AGENT" ? "AGENTE" : senderStr.includes("HUMAN") ? "CLÍNICA" : "PACIENTE";
        ...
        const prompt = `Você é o Agente Clínica, analista de inteligência de uma clínica médica geral.
Sua missão: ler o histórico desta conversa no WhatsApp e decidir a melhor ação para a secretária.
...
Siga rigorosamente o schema de JSON de saída.`;
        ...
        console.error(`[Clínica Geral AI Analyzer] Erro crítico para o chat ${chatId}:`, error);
    }
}
