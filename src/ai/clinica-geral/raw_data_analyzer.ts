...
function detectStrategy(
  ...
  const perChatKeywords = [
    "cada conversa", "cada chat", "classificar", "classificacao",
...
function buildBatches(
  ...
  for (const chatId of Object.keys(byChat).map(Number)) {
    ...
    // Limitar a 80 msgs por chat pra classificacao (suficiente pra entender desfecho)
...
// ─────────────────────────────────────────────────────────────────────────────
// FAN-OUT: Analise paralela por lotes
// ─────────────────────────────────────────────────────────────────────────────
...
async function analyzeBatch(
  ...
  const prompt = `Voce e um analista de atendimento de clinica medica. Classifique CADA conversa.
...
- objecao_logistica: chuva, distancia, transporte
- objecao_hesitacao: vai pensar, falar com alguem
- sem_resposta: clinica nao respondeu ou paciente sumiu
- informativo: so perguntou valor/horario
- retorno: paciente existente voltando
- venda_produto: compra de produto (nao consulta)
- outro: nao se encaixa

REGRAS:
1. Retorne EXATAMENTE ${batch.chats.length} classificacoes (uma por conversa)
2. Use o chat_id exato
3. Baseie-se APENAS no texto fornecido -- NAO invente
...
// ─────────────────────────────────────────────────────────────────────────────
// AGREGACAO (deterministica, sem LLM)
// ─────────────────────────────────────────────────────────────────────────────
...
function formatFanOutResult(
  ...
  return `Periodo: ${period.start} a ${period.end}
Base: ${totalMessages} mensagens brutas de ${agg.total} conversas classificadas
Metodo: Fan-Out Classificacao (${totalBatches} lotes paralelos, 5 simultaneos)
${errors > 0 ? `${errors} lotes com erro (resultados parciais)` : "Todos os lotes processados com sucesso"}

---

## Funil de Conversao (por categoria)
...
## Objecoes Mapeadas (por frequencia)

| Objecao | Ocorrencias | Chats |
| :--- | :---: | :--- |
${objTable}
...
## Citacoes para Verificacao
${topCitations}

---

## Dados Brutos (JSON)
As ${agg.total} classificacoes individuais estao disponiveis no JSON abaixo para cruzamento:
...
export const analyzeRawConversationsTool = new DynamicStructuredTool({
  ...
  description: `Le e analisa as mensagens BRUTAS diretamente da tabela chat_messages (schema atendimento).
Use para QUALQUER analise qualitativa: objecoes, padroes de atendimento, script da secretaria,
sentimento dos pacientes, gargalos, funil de conversao, etc.

DOIS MODOS DE USO:
1. Por PERIODO (padrao): start_date + end_date -- analisa todos os chats do periodo
2. Por LISTA DE IDs: chat_ids=[1234, 5678, ...] -- analisa EXATAMENTE esses chats (mais preciso e rapido)
   -> Use quando ja tiver os IDs de um subset (ex: os 36 chats de objecao_vaga da analise anterior)
   -> Obter IDs: execute_sql("SELECT chat_id FROM atendimento.chat_insights WHERE 'objecao_vaga' = ANY(objecoes)")

IMPORTANTE: per_chat_classification=true ativa fan-out paralelo por conversa (melhor para funil).`,

  schema: z.object({
    start_date: z.string().describe("Data inicio YYYY-MM-DD (BRT). Ignorado se chat_ids for fornecido."),
    end_date: z.string().describe("Data fim YYYY-MM-DD (BRT). Ignorado se chat_ids for fornecido."),
    analysis_goals: z.array(z.string()).describe("Lista de objetivos de analise"),
    ...
    chat_ids: z.array(z.number()).optional()
      .describe("Lista de IDs de chats especificos para analisar. Quando fornecido, ignora start_date/end_date e analisa EXATAMENTE esses chats. Use quando ja tiver uma lista de IDs (ex: os 36 chats de objecao_vaga)."),
  }),
  ...
  func: async ({ start_date, end_date, analysis_goals, sender_filter, include_metadata, per_chat_classification, chat_ids }) => {
   try {
    ...
    // ── PASSO 1: Carregar mensagens brutas com paginacao ──
    let allMessages: RawChatMessage[] = [];
    let offset = 0;
    const BATCH = 1000;

    // Modo chat_ids: busca diretamente pelos IDs especificos (mais preciso e rapido)
    if (chat_ids && chat_ids.length > 0) {
      console.log(`[analyze_raw] chat_ids mode: ${chat_ids.length} chats especificos`);
      while (true) {
        let query = supabase
          .from("chat_messages")
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // ESTRATEGIA A: FAN-OUT (classificacao individual de cada conversa)
    // Sem cap de chats — analisa TODOS. Concorrencia alta (30) compensa volume.
    // ═══════════════════════════════════════════════════════════════════════
    if (strategy === "fan_out_classify") {
      ...
      console.log(`[fan_out] Concluido: ${classifications.length} classificados, ${errors} erros`);
      return formatFanOutResult(classifications, errors, batches.length, allMessages.length, { start: start_date, end: end_date });
    }

    // ═══════════════════════════════════════════════════════════════════════
    // ESTRATEGIA B: SINGLE PASS (analise agregada em uma chamada)
    // ═══════════════════════════════════════════════════════════════════════
    const model = new ChatGoogleGenerativeAI({
      model: "gemini-3.1-flash-preview",
      apiKey: process.env.GEMINI_API_KEY || process.env.GOOGLE_API_KEY,
    ...
    const ANALYSIS_PROMPT = `Voce e uma analista de dados especializada em atendimento medico.

OBJETIVOS DA ANALISE:
${goalsFormatted}

REGRAS ABSOLUTAS:
1. Use APENAS os dados fornecidos. NAO invente.
2. Cite mensagens reais como evidencia (trecho exato entre aspas).
3. Preserve links [[chat:ID|Nome]].
4. Quantifique tudo: "X de Y conversas mencionaram..."
5. Se nao encontrar dados, diga claramente.
6. Organize com secoes em Markdown, UMA POR OBJETIVO.
7. Inclua "Resumo Quantitativo" ao final.
8. Inclua "CITACOES PARA VERIFICACAO" com 5 citacoes: [chat_id]|[trecho]|[sender]
...
`;
    ...
    if (approxTokens <= SINGLE_PASS_LIMIT) {
      ...
      try {
        const response = await model.invoke([
          ...
          { role: "user", content: `TRANSCRICAO (${allMessages.length} msgs):\n\n${transcript}\n\nAnalise conforme os objetivos.` },
        ]);
      ...
      // ═══════════════════════════════════════════════════════════════════
      // ESTRATEGIA C: CHUNKED MAP-REDUCE
      // ═══════════════════════════════════════════════════════════════════
      ...
      const chunkResults = await Promise.all(
        chunks.map(async (chunk, i) => {
          ...
          const response = await model.invoke([
            ...
            { role: "user", content: `TRANSCRICAO (chunk ${i + 1}, ${chunk.length} msgs):\n\n${transcript}` },
          ]);
      ...
      const reduceResponse = await model.invoke([
        { role: "system", content: `Consolide resultados de ${chunks.length} chunks.\n\nOBJETIVOS:\n${goalsFormatted}\n\nREGRAS: Some quantidades, nao duplique, preserve citacoes [[chat:ID|Nome]].` },
    ...
    return `Periodo: ${start_date} a ${end_date}
Base: ${allMessages.length} msgs de ${uniqueChatsCount} conversas
Metodo: ${method}
~${approxTokens.toLocaleString()} tokens | Citacoes: ${citations.length}
