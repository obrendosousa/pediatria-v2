// SOURCE: https://github.com/Kuberwastaken/claurst/blob/main/src-rust/crates/tools/src/send_message.rs
...
impl Tool for SendMessageTool {
    ...
    fn input_schema(&self) -> Value {
        json!({
            ...
            "properties": {
                "to": {
                    "type": "string",
                    "description": "Recipient agent name or \"*\" to broadcast."
                },
                "message": {
                    "type": "string",
                ...
                "summary": {
                    "type": "string",
                    "description": "5-10 word preview (optional)"
                }
            },
            "required": ["to", "message"]
        })
    ...
    async fn execute(&self, input: Value, ctx: &ToolContext) -> ToolResult {
        ...
        let preview = params.summary.as_deref()
            .unwrap_or_else(|| {
        ...
        if params.to == "*" {
            ...
            if recipients.is_empty() {
                return ToolResult::success("Broadcast queued (no active recipients).".to_string());
            }
            for key in &recipients {
                INBOX.entry(key.clone()).or_default().push(msg.clone());
            }
            return ToolResult::success(format!("Broadcast to {} agent(s): {}", recipients.len(), preview));
        }
